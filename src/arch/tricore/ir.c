...
void ir_init_ipi(void)
{
    ...
    apu_enable_access_all_cpus(&ir_int->ACCENSRB[HYP_GSPR_GROUP]);
    apu_enable_access_vm(&ir_int->ACCENSRB[HYP_GSPR_GROUP], HYP_VMID);
...
void ir_send_ipi(cpuid_t target_cpu)
{
    if (ipi_initialized) {
        ...
        ir_int->SRB[HYP_GSPR_GROUP] = 0x1UL << target_cpu;
    }
}

void ir_assign_int_to_vm(unsigned long vmid, irqid_t id)
{
    /* VM direct injection */
    IR_SRC_SET_VM(ir_src->SRC[id], vmid);
}

void ir_assign_icu_to_vm(unsigned long cpuid, unsigned long vmid)
{
    ...
    for (unsigned long int i = 0; i < platform.cpu_num; i++) {
        if (vm->cpus & (1UL << i)) {
            ...
            for (int node = 0; node < 8; node++) {
                ...
                ir_int->GPSRG[id].SWCACCEN[node] |= ACCESSEN_VM_MASK(vm->id)
