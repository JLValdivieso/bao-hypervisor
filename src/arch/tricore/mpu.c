...
static void mpu_prs_set_perm(unsigned long prs, mpid_t mpid, unsigned long perm)
{
    if (perm & PERM_W) {
...
static void mpu_prs_clr_perm(unsigned long prs, mpid_t mpid, unsigned long perm)
{
    if (perm & PERM_W) {
...
bool mpu_map(struct addr_space* as, struct mp_region* mpr, bool locked)
{
    ...
    if (mpr->mem_flags.write || mpr->mem_flags.read) {
        if (existing_data != INVALID_MPID) {
            ...
            mpu_prs_set_perm(prs, existing_data, CAT_RW_FLAGS(mpr->mem_flags));
            entry_added = true;
            ...
            if (existing_code != INVALID_MPID && mpr->mem_flags.exec) { // We assume they were
                ...
                mpu_prs_set_perm(prs, existing_code, PERM_X);
                return true;
            }
        } else {
            ...
            if (data_mpid != INVALID_MPID) {
                ...
                mpu_prs_set_perm(prs, data_mpid, (unsigned long int)CAT_RW_FLAGS(mpr->mem_flags));
    ...
    if (mpr->mem_flags.exec) {
        if (existing_code != INVALID_MPID) {
            ...
            if (code_mpid != INVALID_MPID) {
                ...
                mpu_prs_set_perm(prs, code_mpid, PERM_X);
    ...
    for (mpid_t i = 0; i < MPU_DATA_MAX_NUM_ENTRIES; i++) {
        if (bitmap_get(cpu()->arch.mpu.data_bitmap, i) == 1) {
            if (csfr_dpr_l_read(i) == mpr->base && csfr_dpr_u_read(i) == mpr->base + mpr->size) {
                ...
                mpu_prs_clr_perm(prs, i, PERM_RW);
